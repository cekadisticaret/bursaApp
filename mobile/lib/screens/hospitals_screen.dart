...
class _HospitalsScreenState extends State<HospitalsScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final data = await auth.api.hospitals(ilce: _ilce, band: _band);
      if (!mounted) return;
      setState(() => _data = data);
    } catch (e) {
      if (mounted) {
  ...
  Widget build(BuildContext context) {
    ...
    final privateCount = _data['private_count'] is int ? _data['private_count'] as int : 0;
              ...
              children: [
                ...
                Padding(
                  ...
                  child: Row(
                    children: [
                      Expanded(
                        child: Text(
                          '$total hastane · $groupCount ilçe${_band.isNotEmpty ? ' · $_band' : ''}',
                ...
                if (_loading && groups.isEmpty)
                  ...
                  const Padding(
                    ...
                    child: Center(child: Text('Bu filtrede hastane yok.', style: TextStyle(color: AppColors.muted))),
                  )
                else
                  ...groups.expand((g) {
            ...
            child: Column(
              ...
              children: [
                ...
                Wrap(
                  ...
                  runSpacing: 8,
                  children: [
                    _StatChip(label: '$total hastane'),
                    _StatChip(label: '$groupCount ilçe'),
                    _StatChip(label: privateCount > 0 ? '$privateCount özel' : 'MHRS'),
                  ],
                ),
                if (note.isNotEmpty) ...[
                  const SizedBox(height: 8),
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          Text('$count hastane', style: const TextStyle(color: AppColors.muted, fontWeight: FontWeight.w700, fontSize: 12)),
        ],
      ),
    );
  }
...
String _metaLine(Map<String, dynamic> place) {
  ...
  final type = subLabel.isNotEmpty
      ? subLabel
      : priceBand.isNotEmpty
          ? priceBand
          : hours.isNotEmpty
              ? hours
              : '';
  final parts = <String>[if (type.isNotEmpty) type];
  if (address.isNotEmpty) {
    parts.add(address);
  } else if (ilce.isNotEmpty) {
    parts.add(ilce);
  }
  return parts.join(' · ');
}
...
class _HospitalRow extends StatelessWidget {
  const _HospitalRow({required this.place});
  ...
  Widget build(BuildContext context) {
    ...
    final title = place['title']?.toString() ?? 'Hastane';
    ...
    final showMhrs = mhrsUrl.isNotEmpty && (priceBand == 'Devlet' || priceBand.isEmpty);
        ...
        child: Container(
          ...
          child: Row(
            ...
            children: [
              ...
              if (hours.isNotEmpty || rating.isNotEmpty || priceBand.isNotEmpty)
                Padding(
...
class _Thumb extends StatelessWidget {
  ...
  static const _tones = [Color(0xFF0EA5E9), Color(0xFF0284C7), Color(0xFF0369A1), Color(0xFF38BDF8)];
...
class _ActionChip extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Material(
      color: ghost ? AppColors.bgSoft : _HospitalRow._sky.withValues(alpha: 0.12),
      borderRadius: BorderRadius.circular(999),
      child: InkWell(
        borderRadius: BorderRadius.circular(999),
        onTap: onTap,
        child: Padding(
          ...
          child: Text(
            ...
            style: TextStyle(
              ...
              color: ghost ? AppColors.ink : _HospitalRow._skyDeep,
            ),
          ),
        ),
      ),
